<x-layout.app>
    <div class="max-w-3xl mx-auto space-y-6">
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-2xl font-bold text-base-content">Input Surat Undangan</h1>
                <p class="text-base-content/60 text-sm">Tambahkan jadwal pemeriksaan baru</p>
            </div>
        </div>
        ...
        <div class="card bg-base-100 shadow-sm border border-base-200">
            <div class="card-body">
                <form method="POST" action="{{ route('admin.surat.store') }}" class="space-y-5">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div class="form-control md:col-span-2">
                            ...
                            <input id="sender" type="text" name="sender" value="{{ old('sender') }}"
                                   class="input input-bordered w-full @error('sender') input-error @enderror"
                                   placeholder="Contoh: Dinas Keuangan Kabupaten..." required>
                        ...
                        <div class="form-control md:col-span-2">
                            ...
                            <input id="kegiatan" type="text" name="kegiatan" value="{{ old('kegiatan') }}"
                                   class="input input-bordered w-full @error('kegiatan') input-error @enderror"
                                   placeholder="Contoh: Pemeriksaan Laporan Keuangan" required>
                        ...
                        <div class="form-control">
                            ...
                            <input id="tempat" type="text" name="tempat" value="{{ old('tempat') }}"
                                   class="input input-bordered w-full @error('tempat') input-error @enderror"
                                   placeholder="Nama gedung / kantor" required>
                        ...
                        <div class="form-control md:col-span-2">
                            ...
                            <div id="auditor-list" class="grid grid-cols-2 sm:grid-cols-3 gap-2 p-3 bg-base-200 rounded-lg min-h-[60px]">
                                <p class="text-sm text-base-content/50 col-span-full text-center py-2">Pilih divisi untuk melihat daftar auditor.</p>
                            </div>
                        </div>
            ...
            container.innerHTML = division.auditors.map(a => `
                <label class="flex items-center gap-2 cursor-pointer p-2 bg-base-100 rounded-lg hover:bg-primary/10 transition-colors">
                    <input type="checkbox" name="auditor_ids[]" value="${a.id}" class="checkbox checkbox-primary checkbox-sm"
                           ${oldAuditorIds.includes(String(a.id)) ? 'checked' : ''}>
                    <span class="text-sm">${a.name}</span>
                </label>
            `).join('');
        }

        // Auto-load if division was previously selected
